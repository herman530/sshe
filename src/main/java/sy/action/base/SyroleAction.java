...
public class SyroleAction extends BaseAction<Syrole> {
	...
	public void grid() {
		...
		grid.setRows(((SyroleServiceI) service).findRoleByFilter(hqlFilter,
				page, rows));
		writeJson(grid);
	}
	...
	public void save() {
		...
		if (data != null) {
			SessionInfo sessionInfo = (SessionInfo) getSession().getAttribute(
					ConfigUtil.getSessionInfoName());
			((SyroleServiceI) service).saveRole(data, sessionInfo.getUser()
					.getId());
	...
	public void doNotNeedSecurity_getRoleByUserId() {
		...
		List<Syrole> roles = ((SyroleServiceI) service)
				.findRoleByFilter(hqlFilter);
		writeJson(roles);
	}
